...
class MulticlassSVM:
    ...
    def __init__(self, mode):
        if mode != 'ovr' and mode != 'ovo':
            raise ValueError('mode must be ovr or ovo')
        self.mode = mode
    ...
    def bsvm_ovr_student(self, X, y):
        '''
        Train One Vs Rest binary classfiers.

        Arguments:
            X, y: training features and labels.

        Returns:
            A dictionary with labels as keys,
    ...
    def bsvm_ovo_student(self, X, y):
        '''
        Train One Vs One binary classfiers.

        Arguments:
            X, y: training features and labels.

        Returns:
            A dictionary with label pairs as keys,
                        and binary SVM models as values.
        '''
        binary_svm = {}
        labels = np.unique(y)
    ...
    def scores_ovo_student(self, X):
        '''
        Compute class scores for One Vs One.
